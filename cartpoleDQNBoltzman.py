...
class DQN_Agent:
    ...
    def __init__(self, env, epsilon_max, epsilon_min, epsilon_decay,
                 ...
                 temperature, min_temperature ,decay_rate):
        ...
        self.main_network = DQN_Network(num_actions=self.action_space.n, input_dim=self.observation_space.shape[0]).to(
            device)
        self.target_network = DQN_Network(num_actions=self.action_space.n,
                                          input_dim=self.observation_space.shape[0]).to(
            device).eval()
    ...
    def decay_temperature(self):
        # Update temperature using linear decay
...
class Model_TrainTest:
    def __init__(self, hyperparams):
        ...
        # Define Env
        self.env = gym.make('CartPole-v1', render_mode="human" if self.render else None)
    ...
    def train(self):
        ...
        for episode in range(1, self.max_episodes + 1):
            ...
            while not done and not truncation:
                action = self.agent.select_action(state.squeeze(), self.temperature)
            ...
            self.e_greedy_history.append(self.agent.temperature)
    ...
    def plot_training(self, episode):
        ...
        if episode == self.max_episodes:
            plt.savefig('./cartPole/boltzman3/reward_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('./cartPole/boltzman3/Loss_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        ...
        plt.plot(self.e_greedy_history, label='Temperature', color='#0000ff', alpha=1)
        plt.xlabel("Episode")
        plt.ylabel("Temperature")

        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('./cartPole/boltzman3/Temperature.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "RL_load_path": f'./cartPole/boltzman3/final_weights' + '_' + '5000' + '.pth',
        "save_path": f'./cartPole/boltzman3/final_weights',
        "save_interval": 500,
        "clip_grad_norm": 3,
        ...
        "memory_capacity": 100_000 if train_mode else 0,
